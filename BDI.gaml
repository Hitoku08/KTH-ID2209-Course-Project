...
global {
	...
	//predicates
	string place_in_range <- "the place i s in range";
	predicate goto_target <- new_predicate("go to target");
	predicate find_place <- new_predicate("find a place");
	predicate decide_target<- new_predicate("decide a target");
	predicate reach_place<- new_predicate(place_in_range);
	predicate change_place <- new_predicate("change a place");
}

species guest skills:[moving,fipa] control: simple_bdi{
	
	//Below are the simulation for the BDI behavior of the guests.
	place cur_place <- nil;
	place target_place <- nil;
	int stay_time <- 0;
	//string state <- "leave";
	
	reflex decide_target when: target_place = nil {
		if (cur_place = nil) {
			int coin <- rnd(1);
